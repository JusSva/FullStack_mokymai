...
// 
    // Reikia dar taip pat is total price numinusuot isimto objekto kaina
    // ChatGPT is still garbage
//
    price_of_an_item = row.children[3].textContent.slice(1)
    amount = row.children[2].children[0].value
    total_price = document.querySelector(".total_price")
    cost = total_price.textContent.splice(1, total_price.length) - (amount * price_of_an_item)
    document.querySelector(".total_price").innerHTML = `$${cost.toFixed(2)}`;
    row.remove()
}